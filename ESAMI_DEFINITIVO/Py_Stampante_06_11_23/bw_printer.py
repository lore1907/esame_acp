...
class myListener(stomp.ConnectionListener): 
    ...
    def on_message(self, frame): 
        ...
        if self.input_string in messaggio_ricevuto: 
            print(f"[BW PRINTER LISTENER] Ricevuto messaggio {messaggio_ricevuto}")
            with open("bw.txt", "a") as f: 
                f.write(f"{messaggio_ricevuto} \n")
        else: 
            print(f"[BW PRINTER LISTENER] Il messaggio non contiene l'input desiderato, : {messaggio_ricevuto}")
